...
function createRouter10(db) {
  ...
  router.post('/Facilities', (req, res, next) => {
    db.query(
      ...
      [req.body.FacilityID, req.body.Name, req.body.Street, req.body.Housenumber, req.body.Zipcode, req.body.City, req.body.CountriesID, req.body.FacilityTypsID],
      (error) => {
        if (error) {
          console.error(error);
  ...
  router.put('/Facilities/:FacilityID', function (req, res, next) {
    db.query(
      'UPDATE Facilities SET Name=?, Street=?, Housenumber=?, Zipcode=?, City=?, CountriesID=?, FacilityTypsID=? WHERE FacilityID=?',
      [req.body.FacilityID, req.body.Name, req.body.Street, req.body.Housenumber, req.body.Zipcode, req.body.City, req.body.CountriesID, req.body.FacilityTypsID],
      (error) => {
        if (error) {
          res.status(500).json({status: 'error'});
